...
register 'menu.job.release' => { label => _loc('Releases') };
register 'menu.job.release.new' => { label => _loc('Create a new Release'), url=>'/release/new', title=>_loc('New Release') };
register 'menu.job.release.list' => { label => _loc('List Releases'), url_comp => '/release/list', title=>_loc('Releases') };
...
sub release_contents :Private {
	...
	my @items;
	while( my $r = $rs->next ) {
		push @items, $r->item . " (" . $c->localize($r->provider) . ")";
...
sub release_json : Path('/release/list/json') {
	...
	my $rs = $c->model('Baseliner::BaliRelease')->search(undef, { order_by => $sort ? "$sort $dir" : undef });
	my @rows;
	while( my $r = $rs->next ) {
        next if( $query && !query_array($query, $r->name, $r->description, $r->bl ));
		$c->stash->{id} = $r->id;
		$c->forward('/release_contents');
        push @rows,
          {
            ...
            contents    => '<li>' . join( '<li>', @{ $c->stash->{item_list} || [] } )
